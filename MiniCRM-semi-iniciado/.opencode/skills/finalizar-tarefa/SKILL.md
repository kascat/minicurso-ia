...
6. Pergunte se deve criar uma branch nova. Em caso afirmativo, use o identificador da tarefa seguido do padrão de mercado — um prefixo como `feat/`, `fix/`, `chore/` etc. com uma descrição breve separada por `/` (ex.: a branch `T-03-feat/implementacao-formulario-leads`).
...
9. Pergunte se deve criar a descrição da entrega para a revisão de código (ex.: descrição da PR no GitHub). Em caso afirmativo, salve em `entregas/<nome-da-branch-com-a-barra-trocada-por-traco>.md` (ex.: a branch `T-03-feat/implementacao-formulario-leads` gera `entregas/T-03-feat-implementacao-formulario-leads.md`).
10. Ao final, reporte o resultado dos testes e o hash do commit.